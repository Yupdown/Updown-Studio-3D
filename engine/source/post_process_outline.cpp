...
namespace udsdx
{
	PostProcessOutline::PostProcessOutline(ID3D12Device* device, ID3D12GraphicsCommandList* cmdList)
	{
		m_device = device;
		BuildRootSignature();
	}

	void PostProcessOutline::Pass(RenderParam& param)
	{
		...
		pCommandList->SetGraphicsRootDescriptorTable(1, param.Renderer->GetStencilBufferSrv());
		pCommandList->DrawInstanced(6, 1, 0, 0);
	}

	void PostProcessOutline::OnResize(UINT newWidth, UINT newHeight)
	{
		m_width = newWidth;
		m_height = newHeight;
	...
	void PostProcessOutline::BuildResources()
	{
	...
	void PostProcessOutline::BuildDescriptors(DescriptorParam& descriptorParam)
	{
	...
	void PostProcessOutline::RebuildDescriptors()
	{
	...
	void PostProcessOutline::BuildRootSignature()
	{
		{
	...
	void PostProcessOutline::BuildPipelineState()
	{
